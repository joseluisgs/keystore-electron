...
class GUI {
  ...
  init() {
    ...
    this.app_title.innerHTML = 'Almacén de claves ACME (Carga un almacén para comenzar)';
  ...
  updateRegisterList(registers) {
    const reg_list = this.document.getElementById('register-list');
    ...
    for (const key in registers) {
      const reg_element = this.document.createElement('li');
      ...
      reg_element.addEventListener('click', (e) => {
        const active = this.document.getElementsByClassName('active')[0];
    ...
    if (i > 0) {
      const first_key = Object.keys(registers)[0];
